...
void BoussinesqFlowProblem<dim>::apply_bound_preserving_limiter ()
{
    ...
    for (; cell!=endc; ++cell)
    {
        ...
        for (unsigned int q=0; q<n_q_points_xy; ++q)
        {
            ...
            T_cell_area   += composition_fe_values_xy.JxW(q);
            min_composition = std::min<double> (min_composition,
...
/*
         if
        ((T_cell_average >=(EquationData::Max_T+EquationData::err_tol))
                ||
                (T_cell_average <=(EquationData::Min_T-EquationData::err_tol)))
        {
            std::cout << "   " << "The cell average: " << T_cell_average
                      << std::endl;
        }
*/
        ...
        {
            composition_fe_values_yx.reinit (cell);
            composition_fe_values_yx.get_function_values (DG_composition_solution,
                    composition_values_yx);
...
/* 
            ...
            // Need to divide the area of the cell
            T_cell_average/=T_cell_area;
            std::cout << "   " << "The cell average after: " << T_cell_average
                      << std::endl;
...
void BoussinesqFlowProblem<dim>::solve ()
{
    ...
    if (maximal_velocity >= 0.0001)
        ...
        //          composition_degree *
        time_step = CFL*GridTools::minimal_cell_diameter(triangulation) /
                    maximal_velocity;
    else
        //time_step = 1./(1.7*dim*std::sqrt(1.*dim)) /
        //         composition_degree *
...
start_time_iteration:
    ...
    do
    {
        ...
        std::cout << "   Composition old range1: "
        ...
        std::cout << "   Composition new range1: "
                  << min_composition << ' ' << max_composition
                  << std::endl;
#ifdef AMR
        if ((timestep_number == 0) &&
    ...
    std::cout << "after refine" << std::endl;
        ...
        else if ((timestep_number > 0) && (timestep_number % 1 == 0))
        {
            ...
            std::cout << "   Composition  range1 after refine mesh: "
                      << min_composition << ' ' << max_composition
                      << std::endl;
            apply_bound_preserving_limiter();
        }
        ...
        std::cout << "   Composition range after refine mesh and BPL: "
                  << min_composition << ' ' << max_composition
                  << std::endl;
        std::cout << "   Composition mass integral: "
                  << global_compositional_integrals
