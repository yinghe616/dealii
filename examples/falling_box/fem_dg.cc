    ...
    TrilinosWrappers::BlockVector &coef)
{
  ...
  //const QGaussLobatto<dim> quadrature_formula(composition_degree+1);
    ...
    TrilinosWrappers::BlockVector &coef)
{
  ...
  typename DoFHandler<dim>::active_cell_iterator stokes_cell(&(dinfo1.cell->get_triangulation()),
      ...
      &stokes_dof_handler);
  //const unsigned int n_q_points = fe_v.n_quadrature_points;
  //FEFaceValues<dim> stokes_fe_values(stokes_fe,fe_v.get_quadrature_formula(),update_values);
  /*
     const QGauss<dim> quadrature_formula(composition_degree+1);
     const unsigned int n_q_points = quadrature_formula.size();
     FEValues<dim> stokes_fe_values(stokes_fe,quadrature_formula,update_values);
  ...
  //const QGaussLobatto<dim-1> quadrature_formula(composition_degree+1);
  const unsigned int n_q_points = quadrature_formula.size();
...
void BoussinesqFlowProblem<dim>::setup_dofs ()
{
    ...
    // some time and memory.
    {
        ...
        DynamicSparsityPattern csp(composition_dof_handler.n_dofs());
...
void BoussinesqFlowProblem<dim>::apply_bound_preserving_limiter ()
{
    ...
    for (; cell!=endc; ++cell)
    {
        ...
        )
        {
            ...
            {
                T_cell_average+= composition_values_yx[q]
                                 *composition_fe_values_yx.JxW(q);
                T_cell_area+= 1.0
                              *composition_fe_values_yx.JxW(q);
            min_composition = std::min<double> (min_composition,
                                                composition_values_xy[q]);
...
void BoussinesqFlowProblem<dim>::run ()
{
    ...
    const unsigned int n_pre_refinement_steps = (dim == 2 ? 3 : 3);
 ...
 //   triangulation.refine_global (initial_refinement);
    triangulation.refine_global (6); //if want to use global uniform mesh
